...
`/api/v1`
- Authentication is required for most of the endpoints. You must first login to get a JWT token, which should be included in all subsequent requests in the Authorization header as a Bearer token.
...
**Method**: `POST`\
**Description**: Creates a new account for the authenticated user with an initial deposit.
...
**Endpoint**: `/account/transaction/charge`\
**Method**: `POST`\
...
**Endpoint**: `/account/transaction/deduction`\
**Method**: `POST`\
...
**Endpoint**: `/account/transaction/list`\
**Method**: `GET`\
**Description**: Retrieves the user's account and transaction history.
...
**Method**: `DELETE`\
**Description**: Deletes the user's account and all related data.
...
1.  **Run the H2 Database**:
    ...
    -   You can access the H2 console at `http://localhost:8080/h2-console` after the application is running.
